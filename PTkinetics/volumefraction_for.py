    ...
    def lmbdE(t):
        if t>0 and (include_hom or include_disloc):
            if include_disloc and include_hom:
                out = quad(lambda tp: integrand_hom(tp,t,Pdot,rpref,Ndotpref,epshom,Ttarget)+integrand_dis(tp,t,Pdot,rpref,Ndotpref,epshom,rhob2,alpha_dis,Ttarget), 0, t, epsabs=quadepsabs, epsrel=quadepsrel, limit=quadlimit)
            elif include_hom:
