...
if OPTIONS is not False:
    ...
    if OPTIONS.get('cmax'): OPTIONS.pop('cmax') ## not implemented in fortran version
...
if __name__ == '__main__':
    ...
    rpref,epshom,Ndotpref,rhob2,alpha_dis = compute_prefactors(deltagpprime=DeltaGPprime300,rhomean=rhomean_coex300,gammaam=gammaAM,rhodis=rhodis,burgers=burgers,
    ...
    def lambdaE(t,Pdot):
        ...
        return lambdaE_hd(t=t, Pdot=Pdot, rpref=rpref, Ndotpref=Ndotpref, epshom=epshom,rhob2=rhob2,alpha_dis=alpha_dis,Ttarget=Ttarget,include_hom=include_hom, include_disloc=include_disloc)
    ...
    def maincomputations(xi,pi):
        ...
        if pi>0 and np.isclose(onsetpressure,pressure[-1]):
    ...
    if onsetpressure is not None:
        plot_Vfrac_P_Pdot(res,pressure,pdotvals,figtitle=figtitle,ylabel=ylabel,extendednamestring=extendednamestring,figsize=figsize,xlimits=xlimits,every=every)
    ...
    plot_relaxtime(tau=tau, taumin=taumin, tauplus=tauplus, pdotvals=pdotvals2, extendednamestring=extendednamestring,figtitle=figtitle_pos)
    ...
    plot_onsetP(onsetpressure_pos,pdotvals=pdotvals_pos,figtitle=figtitle_pos,extendednamestring=extendednamestring,linearfit=linearfit,nonlinfit=nonlinfit)
    plot_onsetP2(onsetpressure,pdotvals=pdotvals,figtitle=figtitle,extendednamestring=extendednamestring,Ptrans=Ptransition)
